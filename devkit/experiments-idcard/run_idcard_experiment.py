...
DISTRACTOR_LIST_BASENAME = os.path.join(
    '..', 'templatelists', 'idDistractor_features_list.json')
PROBE_LIST_BASENAME = os.path.join(
    '..', 'templatelists', 'idProbe_features_list.json')
...
def main():
    ...
    parser.add_argument(
        'file_ending', help='Ending appended to original photo files. i.e. 11084833664_0.jpg_LBP_100x100.bin => _LBP_100x100.bin')
    parser.add_argument(
    ...
    parser.add_argument(
        '-ns', '--num_sets', help='Set to change number of sets to run on. Default: 1')
    ...
    parser.add_argument(
        '-p', '--probe_list', help='Set to use different probe list. Default: ../templatelists/scene_idcard_features_list.json')
    parser.add_argument('-dlp', '--distractor_list_path',
                        help='Set to change path used for distractor lists')
    parser.set_defaults(model=MODEL, num_sets=1, sizes=[10, 100, 1000, 10000, 100000, 1000000],
                        probe_list=PROBE_LIST_BASENAME, distractor_list_path=os.path.dirname(DISTRACTOR_LIST_BASENAME))
    ...
    distractor_list_basename = os.path.join(
        args.distractor_list_path, os.path.basename(DISTRACTOR_LIST_BASENAME))
    ...
    distractor_name = os.path.basename(distractor_list_basename).split('_')[0]

    # Create feature lists for megaface for all sets and sizes and verifies
    # all features exist
    missing = False
    for index in set_indices:
        for size in sizes:
            print('Creating feature list of {} photos for set {}'.format(
                size, str(index)))
            cur_list_name = distractor_list_basename + \
                "_{}_{}".format(str(size), str(index))
            with open(cur_list_name) as fp:
                featureFile = json.load(fp)
                path_list = featureFile["path"]
                for i in range(len(path_list)):
                    path_list[i] = os.path.join(
                        distractor_feature_path, path_list[i] + file_ending)
    ...
    with open(probe_list_basename) as fp:
        ...
        for i in range(len(path_list)):
            path_list[i] = os.path.join(
                probe_feature_path, path_list[i] + file_ending)
            if(not os.path.isfile(path_list[i])):
                print path_list[i] + " is missing"
                missing = True
        featureFile["path"] = path_list
